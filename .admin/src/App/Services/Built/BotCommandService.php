...
class BotCommandService
{
    ...
    public function __construct($request, $response, $args )
    {
        ...
        // legacy
        $this->request = $args;
        $this->Form = new BotCommandFormWrapper($request, $args);
    }
        ...
        switch($this->request['a']){
            ...
            case 'view':
                if ($this->request['a'] === 'edit' && empty($this->request['ui'])) {
                    ...
                    $this->content['onReadyJs'] .= "if(window.gcScreens){history.replaceState({gcEdit:null},'',_SITE_URL+'BotCommand/');gcScreens.openEdit('BotCommand'," . $gcEditPk . ");}";
                } else {
    ...
    */
    public function getApiResponse()
    {
        ...
        $Api = new Api('BotCommand', $this, ['IdGridRun', 'Command', 'CmdStatus', 'Note']);
  ...
  array (
    ...
    public function deleteOne()
    {
        ...
        $obj = $_SESSION[_AUTH_VAR]->loadPkScoped(BotCommandQuery::class, json_decode($this->request['i']), 'BotCommand', 'd');
        if ($obj) {
    ...
    public function saveUpdate(): array
    {
        ...
        $data['i'] = ( $data['IdBotCommand'] ) ? $data['IdBotCommand'] : $this->request['i'];
        $data['ip'] = urldecode($this->request['data']['ip'] ?? '');
        $data['pc'] = urldecode($this->request['data']['pc'] ?? '');
        $this->BotCommand['request'] = $this->request;

        if(!empty($data['i'])) {
            ## Save

            $e = $this->Form->setUpdateDefaultsBotCommand($data);
            if ($e === null) {
            ...
            $e = $this->Form->setCreateDefaultsBotCommand($data);
    ...
    */
    ...
    {
        $this->BotCommand['request'] = $this->request;
        $this->BotCommand['parentId'] = $this->request['data']['ip'];
