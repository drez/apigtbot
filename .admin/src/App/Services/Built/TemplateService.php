...
class TemplateService
{
     ...
     * @var string
     */
    public $virtualClassName = 'Template';
    public $rawRequest;
    public $Form;
    public $contentType;
    public $headers;
        ...
        switch($this->request['a']){
            ...
            case 'view':
                if ($this->request['a'] === 'edit' && empty($this->request['ui'])) {
                    ...
                    $this->content['onReadyJs'] .= "if(window.gcScreens){history.replaceState({gcEdit:null},'',_SITE_URL+'Template/');gcScreens.openEdit('Template'," . $gcEditPk . ");}";
                } else {
    ...
    */
    public function getApiResponse()
    {
        ...
        $Api = new Api('Template', $this, ['Name', 'Subject', 'Color1', 'Color2', 'Color3', 'Status', 'Body', 'Footer', 'Lang']);
    ...
    public function deleteOne()
    {
        ...
        $obj = $_SESSION[_AUTH_VAR]->loadPkScoped(TemplateQuery::class, json_decode($this->request['i']), 'Template', 'd');
        if ($obj) {
    ...
    protected function gcDeleteRow($obj, &$error, &$messages)
    {
        ...
        try {
            ...
            error_log('Template delete refused by a foreign key: ' . $gcDelMsg);
