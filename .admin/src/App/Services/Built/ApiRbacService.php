...
class ApiRbacService
{
     ...
     * @var string
     */
    public $virtualClassName = 'ApiRbac';
    public $rawRequest;
    public $Form;
    public $contentType;
    public $headers;
        ...
        switch($this->request['a']){
            ...
            case 'view':
                if ($this->request['a'] === 'edit' && empty($this->request['ui'])) {
                    ...
                    $this->content['onReadyJs'] .= "if(window.gcScreens){history.replaceState({gcEdit:null},'',_SITE_URL+'ApiRbac/');gcScreens.openEdit('ApiRbac'," . $gcEditPk . ");}";
                } else {
    ...
    */
    public function getApiResponse()
    {
        ...
        $Api = new Api('ApiRbac', $this, ['Description', 'Model', 'Action', 'Body', 'Query', 'Method', 'Scope', 'Rule', 'Count']);
    ...
    public function deleteOne()
    {
        ...
        $obj = $_SESSION[_AUTH_VAR]->loadPkScoped(ApiRbacQuery::class, json_decode($this->request['i']), 'ApiRbac', 'd');
        if ($obj) {
    ...
    protected function gcDeleteRow($obj, &$error, &$messages)
    {
        ...
        try {
            ...
            error_log('ApiRbac delete refused by a foreign key: ' . $gcDelMsg);
