                    ...
                    foreach($order as $col => $sens){
                        if($sens){
                            ...
                            $this->orderReadyJsOrder .="
                                var __se=document.querySelector(\"#SimWalletListForm [th='sorted'][c='".$col."']\");if(__se){__se.setAttribute('sens', '".strtolower($sens)."');__se.setAttribute('order','on');__se.classList.add('sorted');}
                            ";
                        }
                        $f++;
                    }
...
. $this->cCmoreColsHeader;
  ...
  'IdModification' => NULL,
        ...
        // if Search params
        $this->searchMs = $this->setSearchVar($request['ms'] ?? '', 'SimWallet/');
        ...
        // order
        $this->searchOrder = $this->setOrderVar($request['order'] ?? '', 'SimWallet/');
        ...
        $gcSortClear = '';
        $gcSortSheetClear = '';
        if (!empty($_SESSION['mem']['order']['SimWallet/'])) {
        ...
        // page
        $search['page'] = $this->setPageVar($request['pg'] ?? '', 'SimWallet/');
...
. $actionCell
        ...
        $return['html'] =
            ...
            .div(
                ...
                .$this->hookListBottom
                .$bottomRow
            , 'SimWalletListForm', " class='va-mob proto-app' data-model='SimWallet' data-table='SimWallet' data-ui='".$this->uiTabsId."' ");
    ...
    */
    ...
    public function getEditForm($id, $uiTabsId = 'tabsContain', $data=[], $error=[], $jsElement='', $jsElementType=''): array
    {
        ...
        if($_SESSION[_AUTH_VAR]->hasRights('SimWallet', 'a') && !$this->setReadOnly) {
            $this->formAddButton = htmlLink(_("Add new"), 'Javascript:;' , "id='addSimWallet' title='"._('Add')."' class='button-link-blue add-button'");
            $this->bindEditJs = "";
...
.$this->fields['SimWallet']['Qty']['html'] ."</div>",'',"class='form-card'")
        ...
        if($_SESSION['mem']['SimWallet']['ixmemautocapp'] and $_GET['Autocapp'] == 1) {
            $Autocapp = $_SESSION['mem']['SimWallet']['ixmemautocapp'];
            unset($_SESSION['mem']['SimWallet']['ixmemautocapp']);
        }
    ...
    function lockFormField($fields, $dataObj)
    {
        ...
        if($fields == 'all') {
            foreach($this->fields['SimWallet'] as $field => $ar) {
