...
class AuthyGroupXService
{
     ...
     * @var string
     */
    public $virtualClassName = 'AuthyGroupX';
    public $rawRequest;
    public $Form;
    public $contentType;
    public $headers;
        ...
        switch($this->request['a']){
            ...
            case 'view':
                if ($this->request['a'] === 'edit' && empty($this->request['ui'])) {
                    ...
                    $this->content['onReadyJs'] .= "if(window.gcScreens){history.replaceState({gcEdit:null},'',_SITE_URL+'AuthyGroupX/');gcScreens.openEdit('AuthyGroupX'," . $gcEditPk . ");}";
                } else {
    ...
    */
    public function getApiResponse()
    {
        ...
        $Api = new Api('AuthyGroupX', $this, null);
    ...
    public function deleteOne()
    {
        ...
        $obj = $_SESSION[_AUTH_VAR]->loadPkScoped(AuthyGroupXQuery::class, json_decode($this->request['i']), 'AuthyGroupX', 'd');
        if ($obj) {
    ...
    protected function gcDeleteRow($obj, &$error, &$messages)
    {
        ...
        try {
            ...
            error_log('AuthyGroupX delete refused by a foreign key: ' . $gcDelMsg);
