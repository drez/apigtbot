...
class BotEventService
{
    ...
    public function __construct($request, $response, $args )
    {
        ...
        // legacy
        $this->request = $args;
        $this->Form = new BotEventFormWrapper($request, $args);
    }
        ...
        switch($this->request['a']){
            ...
            case 'view':
                if ($this->request['a'] === 'edit' && empty($this->request['ui'])) {
                    ...
                    $this->content['onReadyJs'] .= "if(window.gcScreens){history.replaceState({gcEdit:null},'',_SITE_URL+'BotEvent/');gcScreens.openEdit('BotEvent'," . $gcEditPk . ");}";
                } else {
    ...
    */
    public function getApiResponse()
    {
        ...
        $Api = new Api('BotEvent', $this, ['IdGridRun', 'Level', 'Kind', 'Message', 'Payload']);
  ...
  array (
    ...
    public function deleteOne()
    {
        ...
        $obj = $_SESSION[_AUTH_VAR]->loadPkScoped(BotEventQuery::class, json_decode($this->request['i']), 'BotEvent', 'd');
        if ($obj) {
    ...
    public function saveUpdate(): array
    {
        ...
        $data['i'] = ( $data['IdBotEvent'] ) ? $data['IdBotEvent'] : $this->request['i'];
        $data['ip'] = urldecode($this->request['data']['ip'] ?? '');
        $data['pc'] = urldecode($this->request['data']['pc'] ?? '');
        $this->BotEvent['request'] = $this->request;

        if(!empty($data['i'])) {
            ## Save

            $e = $this->Form->setUpdateDefaultsBotEvent($data);
            if ($e === null) {
            ...
            $e = $this->Form->setCreateDefaultsBotEvent($data);
    ...
    */
    ...
    {
        $this->BotEvent['request'] = $this->request;
        $this->BotEvent['parentId'] = $this->request['data']['ip'];
