...
class TradeCycleService
{
    ...
    public function __construct($request, $response, $args )
    {
        ...
        // legacy
        $this->request = $args;
        $this->Form = new TradeCycleFormWrapper($request, $args);
    }
        ...
        switch($this->request['a']){
            ...
            case 'view':
                if ($this->request['a'] === 'edit' && empty($this->request['ui'])) {
                    ...
                    $this->content['onReadyJs'] .= "if(window.gcScreens){history.replaceState({gcEdit:null},'',_SITE_URL+'TradeCycle/');gcScreens.openEdit('TradeCycle'," . $gcEditPk . ");}";
                } else {
    ...
    */
    public function getApiResponse()
    {
        ...
        $Api = new Api('TradeCycle', $this, ['IdGridRun', 'LevelIdx', 'BuyPrice', 'SellPrice', 'Qty', 'RealizedPnl', 'FeesTotal', 'Simulated']);
  ...
  array (
    ...
    function summaryCards()
    {
        ...
        $scope = $s->hasRights('TradeCycle', 'r');
  ...
  array (
        ...
        foreach ($cards as $c) {
            ...
            if ($c['agg'] === 'count') {
                ...
                if (!isset($gcAggMap[$c['agg']])) {
                    ...
                    $val = (float) $q->withColumn($gcAggMap[$c['agg']] . '(App\\TradeCycle.' . $c['col'] . ')', 'gcAggVal')
                        ->select(['gcAggVal'])
                        ->findOne();
                }
            }
    ...
    public function deleteOne()
    {
        ...
        $obj = $_SESSION[_AUTH_VAR]->loadPkScoped(TradeCycleQuery::class, json_decode($this->request['i']), 'TradeCycle', 'd');
        if ($obj) {
    ...
    public function saveUpdate(): array
    {
        ...
        $data['i'] = ( $data['IdTradeCycle'] ) ? $data['IdTradeCycle'] : $this->request['i'];
        $data['ip'] = urldecode($this->request['data']['ip'] ?? '');
        $data['pc'] = urldecode($this->request['data']['pc'] ?? '');
        $this->TradeCycle['request'] = $this->request;

        if(!empty($data['i'])) {
            ## Save

            $e = $this->Form->setUpdateDefaultsTradeCycle($data);
            if ($e === null) {
            ...
            $e = $this->Form->setCreateDefaultsTradeCycle($data);
    ...
    */
    ...
    {
        $this->TradeCycle['request'] = $this->request;
        $this->TradeCycle['parentId'] = $this->request['data']['ip'];
