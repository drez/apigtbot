...
class CountryService
{
     ...
     * @var string
     */
    public $virtualClassName = 'Country';
    public $rawRequest;
    public $Form;
    public $contentType;
    public $headers;
        ...
        switch($this->request['a']){
            ...
            case 'view':
                if ($this->request['a'] === 'edit' && empty($this->request['ui'])) {
                    ...
                    $this->content['onReadyJs'] .= "if(window.gcScreens){history.replaceState({gcEdit:null},'',_SITE_URL+'Country/');gcScreens.openEdit('Country'," . $gcEditPk . ");}";
                } else {
    ...
    */
    public function getApiResponse()
    {
        ...
        $Api = new Api('Country', $this, ['Name', 'Code', 'Timezone', 'TimezoneCode', 'Priority']);
    ...
    public function deleteOne()
    {
        ...
        $obj = $_SESSION[_AUTH_VAR]->loadPkScoped(CountryQuery::class, json_decode($this->request['i']), 'Country', 'd');
        if ($obj) {
    ...
    protected function gcDeleteRow($obj, &$error, &$messages)
    {
        ...
        try {
            ...
            error_log('Country delete refused by a foreign key: ' . $gcDelMsg);
