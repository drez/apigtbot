...
class ApiLogService
{
    ...
    public function getResponse()
    {
        ...
        switch($this->request['a']){
            ...
            case 'view':
                if ($this->request['a'] === 'edit' && empty($this->request['ui'])) {
                    ...
                    $this->content['onReadyJs'] .= "if(window.gcScreens){history.replaceState({gcEdit:null},'',_SITE_URL+'ApiLog/');gcScreens.openEdit('ApiLog'," . $gcEditPk . ");}";
                } else {
        ...
        case 'prune':
            $this->contentType = 'application/json';
            return json_encode($this->pruneOld(), JSON_UNESCAPED_SLASHES);
    ...
    */
    public function getApiResponse()
    {
        ...
        $Api = new Api('ApiLog', $this, ['IdApiRbac', 'IdAuthy', 'Time', 'RawParameters', 'Count']);
  ...
  '1 day' => '-1 day',
            ...
            // Bulk destructive: require the UNRESTRICTED delete right on the
            // model (hasRights === true; owner/group-scoped grants refuse).
            $sess = $_SESSION[_AUTH_VAR] ?? null;
            if (!is_object($sess) || $sess->hasRights('ApiLog', 'd') !== true) {
                return ['error' => 'yes', 'affected' => 0, 'html' => 'Not allowed'];
            }

            $olderThan = (string) ($this->request['data']['older_than'] ?? '');
            if (!isset($allowed[$olderThan])) {
                return ['error' => 'yes', 'affected' => 0, 'html' => 'Invalid prune window'];
            }
            ...
            return [
                'error' => 'no',
                'affected' => $affected,
                'html' => "Pruned $affected entries older than $olderThan",
            ];
        }
    ...
    public function saveUpdate(): array
    {
        ...
        if(!empty($data['i'])) {
            ...
            if($data['ip']){
                $e->setIdApiRbac(json_decode($data['ip']));
            }
