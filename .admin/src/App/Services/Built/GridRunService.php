...
class GridRunService
{
     ...
     * @var string
     */
    public $virtualClassName = 'GridRun';
    public $rawRequest;
    public $Form;
    public $contentType;
    public $headers;
    ...
    public function __construct($request, $response, $args )
    {
        ...
        // legacy
        $this->request = $args;
        $this->Form = new GridRunFormWrapper($request, $args);
    }
        ...
        switch($this->request['a']){
            ...
            case 'view':
                if ($this->request['a'] === 'edit' && empty($this->request['ui'])) {
                    ...
                    $this->content['onReadyJs'] .= "if(window.gcScreens){history.replaceState({gcEdit:null},'',_SITE_URL+'GridRun/');gcScreens.openEdit('GridRun'," . $gcEditPk . ");}";
                } else {
            ...
            case 'delete':
                ...
                **/
                case 'BotOrder':
            if(!$_SESSION[_AUTH_VAR]->hasRights('BotOrder', 'r')){
                security_redirect(false);
            }
                    $this->content = $this->Form->getBotOrderList($this->request['i'], $this->request);
                break;
                ...
                **/
                case 'TradeCycle':
            if(!$_SESSION[_AUTH_VAR]->hasRights('TradeCycle', 'r')){
                security_redirect(false);
            }
                    $this->content = $this->Form->getTradeCycleList($this->request['i'], $this->request);
                break;
                ...
                **/
                case 'BotEvent':
            if(!$_SESSION[_AUTH_VAR]->hasRights('BotEvent', 'r')){
                security_redirect(false);
            }
                    $this->content = $this->Form->getBotEventList($this->request['i'], $this->request);
                break;
                ...
                **/
                case 'BotCommand':
            if(!$_SESSION[_AUTH_VAR]->hasRights('BotCommand', 'r')){
                security_redirect(false);
            }
                    $this->content = $this->Form->getBotCommandList($this->request['i'], $this->request);
                break;
                ...
                **/
                case 'GridRunAudit':
            if(!$_SESSION[_AUTH_VAR]->hasRights('GridRunAudit', 'r')){
                security_redirect(false);
            }
                    $this->content = $this->Form->getGridRunAuditList($this->request['i'], $this->request);
                break;
    ...
    */
    public function getApiResponse()
    {
        ...
        $Api = new Api('GridRun', $this, ['Label', 'Symbol', 'Status', 'KillSwitch', 'Profile', 'Algo', 'Simulated', 'PLow', 'PHigh', 'NLevels', 'Spacing', 'Allocation', 'BudgetQuote', 'DeployPct', 'AllocMode', 'FeePct', 'MaxPositionQuote', 'MaxOrderQuote', 'DailyLossLimitQuote', 'MaxUnrealizedLossQuote', 'SellAtLoss', 'SellWhenStarved', 'BreakoutBufferPct', 'BreakoutPolicy', 'MaxOpenOrders', 'MaxBuyLevelsBelow', 'TrendTf', 'DonchianPeriod', 'TrendEmaFast', 'TrendEmaSlow', 'AtrPeriod', 'AtrStopMult', 'AtrInitialMult', 'TrendStopFloorPct', 'TrendSignal', 'ReentryCooldown', 'EngineState', 'LastTickAt', 'LastPrice', 'BalBase', 'BalQuote', 'SimBalBase', 'SimBalQuote', 'RunUid', 'AppliedGeometry', 'LedgerResetAt']);
    ...
    protected function gcDeleteRow($obj, &$error, &$messages)
    {
        ...
        try {
            ...
            error_log('GridRun delete refused by a foreign key: ' . $gcDelMsg);
    ...
    public function saveUpdate(): array
    {
        ...
        $data['i'] = ( $data['IdGridRun'] ) ? $data['IdGridRun'] : $this->request['i'];
        ...
        if(!empty($data['i'])) {
            ...
            $e = $this->Form->setUpdateDefaultsGridRun($data);
            if ($e === null) {
                $error = [_('Record not found or access denied')];
            } else {

            if (method_exists($this, 'beforeSave')){ $this->beforeSave($e, $data, false, $messages, $extValidationErr, $error);}
            ...
            $e = $this->Form->setCreateDefaultsGridRun($data);

            if (method_exists($this, 'beforeSave')){ $this->beforeSave($e, $data, true, $messages, $extValidationErr, $error);}
