                    ...
                    foreach($order as $col => $sens){
                        if($sens){
                            ...
                            $this->orderReadyJsOrder .="
                                var __se=document.querySelector(\"#GridRunListForm [th='sorted'][c='".$col."']\");if(__se){__se.setAttribute('sens', '".strtolower($sens)."');__se.setAttribute('order','on');__se.classList.add('sorted');}
                            ";
                        }
                        $f++;
                    }
...
. $this->cCmoreColsHeader;
  ...
  'IdModification' => NULL,
        ...
        // if Search params
        $this->searchMs = $this->setSearchVar($request['ms'] ?? '', 'GridRun/');
        ...
        // order
        $this->searchOrder = $this->setOrderVar($request['order'] ?? '', 'GridRun/');
        ...
        $gcSortClear = '';
        $gcSortSheetClear = '';
        if (!empty($_SESSION['mem']['order']['GridRun/'])) {
        ...
        // page
        $search['page'] = $this->setPageVar($request['pg'] ?? '', 'GridRun/');
...
. $actionCell
        ...
        $return['html'] =
            ...
            .div(
                ...
                .$this->hookListBottom
                .$bottomRow
            , 'GridRunListForm', " class='va-mob proto-app' data-model='GridRun' data-table='GridRun' data-ui='".$this->uiTabsId."' ");
    ...
    */
    ...
    public function getEditForm($id, $uiTabsId = 'tabsContain', $data=[], $error=[], $jsElement='', $jsElementType=''): array
    {
        ...
        if($_SESSION[_AUTH_VAR]->hasRights('GridRun', 'a') && !$this->setReadOnly) {
            $this->formAddButton = htmlLink(_("Add new"), 'Javascript:;' , "id='addGridRun' title='"._('Add')."' class='button-link-blue add-button'");
            $this->bindEditJs = "";
...
$this->fields['GridRun']['Status']['html'] = stdFieldRow(_("Status"), selectboxCustomArray('Status', [ '0' => ['0'=>_("Draft"), '1'=>"Draft"],'1' => ['0'=>_("DryRun"), '1'=>"DryRun"],'2' => ['0'=>_("Testnet"), '1'=>"Testnet"],'3' => ['0'=>_("Live"), '1'=>"Live"],'4' => ['0'=>_("Halted"), '1'=>"Halted"],'5' => ['0'=>_("Done"), '1'=>"Done"], ], "", "s='d'  ", $dataObj->getStatus(), '', false), 'Status', "", $this->commentsStatus, $this->commentsStatus_css, ' half', ' ', 'no', 'v2');
...
$this->fields['GridRun']['LedgerResetAt']['html'] = stdFieldRow(_("Ledger rebased at (daemon-managed)"), input('datetime-local', 'LedgerResetAt', $dataObj->getLedgerResetAt(), "  j='date' autocomplete='off' placeholder='YYYY-MM-DD hh:mm:ss' size='30'  s='d' class='' title='Ledger rebased at (daemon-managed)'"), 'LedgerResetAt', "", $this->commentsLedgerResetAt, $this->commentsLedgerResetAt_css, ' half', ' ', 'no', 'v2');
        ...
        $ongletf =
            div(
                div(button(_('Grid Run'), ' type="button" class="tab-btn is-active" role="tab" data-tab="tab_GridRun" aria-selected="true" aria-controls="tab_GridRun" ')
                    ...
                    .button(_('Telemetry'), ' type="button" class="tab-btn" role="tab" data-tab="tab_last_tick_at" aria-selected="false" aria-controls="tab_last_tick_at" '),'',"class='sw-tabnav' role='tablist'")
        ...
        .form(
            div(
                ...
                .div(
                    ...
                    .div('' .
                    '<div id="tab_GridRun" class="tab-pane is-active" role="tabpanel" data-tab="tab_GridRun"><div class="sw-grid">'.
...
.$this->fields['GridRun']['LedgerResetAt']['html'].'</div></div>' ,'',"class='form-card'")
        ...
        if($_SESSION['mem']['GridRun']['ixmemautocapp'] and $_GET['Autocapp'] == 1) {
            $Autocapp = $_SESSION['mem']['GridRun']['ixmemautocapp'];
            unset($_SESSION['mem']['GridRun']['ixmemautocapp']);
        }
    ...
    function lockFormField($fields, $dataObj)
    {
        ...
        if($fields == 'all') {
            foreach($this->fields['GridRun'] as $field => $ar) {
