...
class GridRunAuditService
{
     ...
     * @var string
     */
    public $virtualClassName = 'GridRunAudit';
    public $rawRequest;
    public $Form;
    public $contentType;
    public $headers;
    ...
    public function __construct($request, $response, $args )
    {
        ...
        // legacy
        $this->request = $args;
        $this->Form = new GridRunAuditFormWrapper($request, $args);
    }
        ...
        switch($this->request['a']){
            ...
            case 'view':
                if ($this->request['a'] === 'edit' && empty($this->request['ui'])) {
                    ...
                    $this->content['onReadyJs'] .= "if(window.gcScreens){history.replaceState({gcEdit:null},'',_SITE_URL+'GridRunAudit/');gcScreens.openEdit('GridRunAudit'," . $gcEditPk . ");}";
                } else {
    ...
    */
    public function getApiResponse()
    {
        ...
        $Api = new Api('GridRunAudit', $this, ['IdGridRun', 'Field', 'ValueFrom', 'ValueTo', 'Actor', 'Source']);
  ...
  array (
    ...
    public function deleteOne()
    {
        ...
        $obj = $_SESSION[_AUTH_VAR]->loadPkScoped(GridRunAuditQuery::class, json_decode($this->request['i']), 'GridRunAudit', 'd');
        if ($obj) {
    ...
    protected function gcDeleteRow($obj, &$error, &$messages)
    {
            ...
            $error = handleNotOkResponse(_("audit history is append-only; rows are written by the audited table's save"), '', true, 'Change history');
        ...
        try {
            ...
            error_log('GridRunAudit delete refused by a foreign key: ' . $gcDelMsg);
    ...
    public function saveUpdate(): array
    {
        ...
        $data['i'] = ( $data['IdGridRunAudit'] ) ? $data['IdGridRunAudit'] : $this->request['i'];
        ...
        if(!empty($data['i'])) {
            ...
            $e = $this->Form->setUpdateDefaultsGridRunAudit($data);
            if ($e === null) {
            ...
            $e = $this->Form->setCreateDefaultsGridRunAudit($data);
