...
class AuthyLogService
{
     ...
     * @var string
     */
    public $virtualClassName = 'AuthyLog';
    public $rawRequest;
    public $Form;
    public $contentType;
    public $headers;
        ...
        switch($this->request['a']){
            ...
            case 'view':
                if ($this->request['a'] === 'edit' && empty($this->request['ui'])) {
                    ...
                    $this->content['onReadyJs'] .= "if(window.gcScreens){history.replaceState({gcEdit:null},'',_SITE_URL+'AuthyLog/');gcScreens.openEdit('AuthyLog'," . $gcEditPk . ");}";
                } else {
    ...
    */
    public function getApiResponse()
    {
        ...
        $Api = new Api('AuthyLog', $this, ['IdAuthy', 'Timestamp', 'Login', 'Userid', 'Result', 'Event', 'Ip', 'Count']);
    ...
    public function deleteOne()
    {
        ...
        $obj = $_SESSION[_AUTH_VAR]->loadPkScoped(AuthyLogQuery::class, json_decode($this->request['i']), 'AuthyLog', 'd');
        if ($obj) {
    ...
    protected function gcDeleteRow($obj, &$error, &$messages)
    {
        ...
        try {
            ...
            error_log('AuthyLog delete refused by a foreign key: ' . $gcDelMsg);
    ...
    public function saveUpdate(): array
    {
        ...
        if(!empty($data['i'])) {
            ...
            if($data['ip']){
                $e->setIdAuthy(json_decode($data['ip']));
            }
