...
class BotOrderService
{
    ...
    public function __construct($request, $response, $args )
    {
        ...
        // legacy
        $this->request = $args;
        $this->Form = new BotOrderFormWrapper($request, $args);
    }
        ...
        switch($this->request['a']){
            ...
            case 'view':
                if ($this->request['a'] === 'edit' && empty($this->request['ui'])) {
                    ...
                    $this->content['onReadyJs'] .= "if(window.gcScreens){history.replaceState({gcEdit:null},'',_SITE_URL+'BotOrder/');gcScreens.openEdit('BotOrder'," . $gcEditPk . ");}";
                } else {
    ...
    */
    public function getApiResponse()
    {
        ...
        $Api = new Api('BotOrder', $this, ['IdGridRun', 'ClientOrderId', 'ExchangeOrderId', 'LevelIdx', 'Side', 'State', 'Price', 'Qty', 'FilledQty', 'FeePaid', 'FeeAsset', 'IsLegacy', 'LegacyBuyPrice', 'LegacyBuyFee', 'Simulated']);
  ...
  array (
    ...
    function summaryCards()
    {
        ...
        $s = $_SESSION[_AUTH_VAR] ?? null;
        if (!is_object($s) || !method_exists($s, 'hasRights')) { return $body; }
        $scope = $s->hasRights('BotOrder', 'r');
        if ($scope === false) { return $body; }
  ...
  array (
    ...
    array (
      'State' => 'Vetoed',
        ...
        $out = [];
        foreach ($cards as $c) {
            $q = \App\BotOrderQuery::create();
            // Tenant + Owner/Group read-scope (mirror the list / autocomplete).
            if (method_exists($s, 'get') && !$s->get('isRoot') && $s->get('id_tenant') && method_exists($q, 'filterByIdTenant')) {
                $q->filterByIdTenant($s->get('id_tenant'));
            }
            if (is_array($scope)) {
                if (in_array('Owner', $scope, true) && method_exists($q, 'filterByIdCreation')) {
                    $q->filterByIdCreation($s->getIdAuthy());
                    if (in_array('Group', $scope, true) && method_exists($q, 'filterByIdGroupCreation')) {
                        $q->_or()->filterByIdGroupCreation($s->getGroups(), \Criteria::IN);
                    }
                } elseif (in_array('Group', $scope, true) && method_exists($q, 'filterByIdGroupCreation')) {
                    $q->filterByIdGroupCreation($s->getGroups(), \Criteria::IN);
                }
            }
            foreach ($c['filter'] as $fcol => $fval) {
                $m = 'filterBy' . $fcol;
                if (!method_exists($q, $m)) { continue; }
                if (is_array($fval)) { $q->$m($fval, \Criteria::IN); } else { $q->$m($fval); }
            }
            if ($c['agg'] === 'count') {
                $val = (float) $q->count();
            } else {
                // Aggregate in SQL — fetching every matching row's column to
                // reduce in PHP scaled with the table size. agg/col come from
                // the build-time card literal; whitelist agg anyway before it
                // is spliced into the SQL clause.
                $gcAggMap = ['sum' => 'SUM', 'avg' => 'AVG', 'min' => 'MIN', 'max' => 'MAX'];
                if (!isset($gcAggMap[$c['agg']])) {
                    $val = 0;
                } else {
                    $val = (float) $q->withColumn($gcAggMap[$c['agg']] . '(App\\BotOrder.' . $c['col'] . ')', 'gcAggVal')
                        ->select(['gcAggVal'])
                        ->findOne();
                }
            }
            $out[] = ['label' => $c['label'], 'value' => $val, 'format' => $c['format']];
        }
        $body['data'] = $out;
        $body['status'] = 'success';
        return $body;
    ...
    public function deleteOne()
    {
        ...
        $obj = $_SESSION[_AUTH_VAR]->loadPkScoped(BotOrderQuery::class, json_decode($this->request['i']), 'BotOrder', 'd');
        if ($obj) {
    ...
    public function saveUpdate(): array
    {
        ...
        $data['i'] = ( $data['IdBotOrder'] ) ? $data['IdBotOrder'] : $this->request['i'];
        $data['ip'] = urldecode($this->request['data']['ip'] ?? '');
        $data['pc'] = urldecode($this->request['data']['pc'] ?? '');
        $this->BotOrder['request'] = $this->request;

        if(!empty($data['i'])) {
            ## Save

            $e = $this->Form->setUpdateDefaultsBotOrder($data);
            if ($e === null) {
            ...
            $e = $this->Form->setCreateDefaultsBotOrder($data);
    ...
    */
    ...
    {
        $this->BotOrder['request'] = $this->request;
        $this->BotOrder['parentId'] = $this->request['data']['ip'];

            if($this->request['data']['ip']){
                $relData['IdGridRun'] = $this->request['data']['ip'];
