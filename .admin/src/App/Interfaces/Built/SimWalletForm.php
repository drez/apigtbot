 ...
 *
 *  @version 1.1
 *  Generated Form class on the 'SimWallet' table.
 *
 */
...
class SimWalletForm extends SimWallet
{
    ...
    */
    ...
    public $orderReadyJsOrder = '';
        ...
        public $commentsIdSimWallet;
    public $commentsIdSimWallet_css;
    public $commentsAsset;
    public $commentsAsset_css;
    public $commentsQty;
    ...
    **/
    ...
    function __construct(Request|array|null $request, array $args)
    {
        ...
        $this->args = $args;
        $this->model_name = 'SimWallet';
        $this->virtualClassName = 'SimWallet';
                        ...
                        if($sens){
                            ...
                            if(!empty($tOrd[1])){
                                ...
                                error_log('list order: dropping unresolvable column ' . (string) $col
                                    . ' on SimWallet — ' . $gcOrdEx->getMessage());
                                unset($_SESSION['mem']['order']['SimWallet/'],
                                    $_SESSION['mem']['order']['SimWallet/child']);
                            }
                            if($gcOrdApplied){
                            ...
                            $this->orderReadyJsOrder .="
                                (function(){var __c=".$gcOrdCol.",__s=".$gcOrdSens.";var __se=document.querySelector(\"#SimWalletListForm [th='sorted'][c=\"+JSON.stringify(__c)+\"]\");if(__se){__se.setAttribute('sens', __s);__se.setAttribute('order','on');__se.classList.add('sorted');}})();
                            ";
                            }
                        }
                        $f++;
    ...
    public function getListHeader(string $act): ?string
    {
        ...
        switch($act) {
            case 'head':
                $trHead = th(_("Asset"), " th='sorted' c='Asset' title='" . _('Asset')."' ")
.th(_("Quantity"), " th='sorted' c='Qty' title='" . _('Quantity')."' ")
. $this->cCmoreColsHeader;
            ...
            case 'add':
            ###### ADD
                if($_SESSION[_AUTH_VAR]->hasRights('SimWallet', 'a') && !$this->setReadOnly){

                                $this->listAddButton = htmlLink(
                                    _("Add new")
                                ,_SITE_URL.$this->virtualClassName."/edit/", "id='addSimWallet' title='"._('Add')."' class='button-link-blue add-button'");
            }
        ...
        if( $pmpoData->isEmpty() ) {
            ...
            **/
            ...
            if(!$this->setReadOnly && !$this->setListRemoveDelete){
                if($_SESSION[_AUTH_VAR]->hasRights('SimWallet', 'd')){
                    $this->canDelete = htmlLink("<i class='ri-delete-bin-7-line'></i>", "Javascript:", "class='ac-delete-link' j='deleteSimWallet' ");
                }
            }
 ...
 ''
 . div(
   div('' . span(htmlspecialchars((string)((($altValue['Asset'] !== null ) ? $altValue['Asset'] : $data->getAsset())))." ", "   i='" . $__pkJsonEsc . "' c='Asset' class=''  j='editSimWallet'") ,''," class='name' ")
   . div(''  . span(htmlspecialchars((string)((($altValue['Qty'] !== null ) ? $altValue['Qty'] : str_replace(',', '.', (string)($data->getQty() ?? '')))))." ", "   i='" . $__pkJsonEsc . "' c='Qty' class='right'  j='editSimWallet'") . $cCmoreCols ,''," class='meta' ")
 ,'', " class='body' ")
...
. div($actionInner, '', " class='actionrow' ")
                , '', "
                        ...
                        id='SimWalletRow".$__pkEsc."'")
                ;
                $gcDtRowHtml = tr(
                td(span(htmlspecialchars((string)((($altValue['Asset'] !== null ) ? $altValue['Asset'] : $data->getAsset())))." "), "  i='" . $__pkJsonEsc . "' c='Asset' class=''  j='editSimWallet'") .
                td(span(htmlspecialchars((string)((($altValue['Qty'] !== null ) ? $altValue['Qty'] : str_replace(',', '.', (string)($data->getQty() ?? '')))))." "), "  i='" . $__pkJsonEsc . "' c='Qty' class='right'  j='editSimWallet'") .  $actionCell, "  rid='".$__pkJsonEsc."' data-iterator='".$pcData->getPosition()."' r='data' class='va-dt-row ".$hook['class']." ' id='SimWalletDtRow".$__pkEsc."'");
                ...
                if ($gcGroupOn) {
                    $gcVal = (string) ((($altValue['Asset'] !== null ) ? $altValue['Asset'] : $data->getAsset()));
            ...
            $tr .= input('hidden', 'rowCountSimWallet', $i);
        ...
        $return['html'] =
            ...
            .div(
                ...
                .div($controlsContent,'SimWalletControlsList', "class='custom-controls'")
            ...
            .div(
                div(
                    div(
                        ...
                        .span(_('Paper Wallet'), " class='title' ")
                        ...
                        .(($_SESSION[_AUTH_VAR]->hasRights('SimWallet', 'a') && !$this->setReadOnly) ? href("<i class='ri-add-line'></i>"._('New'), _SITE_URL.$this->virtualClassName."/edit/", " class='add-btn' ") : '')
                    ,''," class='va-mob-row1' ")
                    .div(
                ...
                .div(
                    ...
                    .div(
                        ...
                        ,''," class='sheet-head' ")
                        .div("".$gcSortSheetClear . button(_("Asset"), " type='button' th='sorted' c='Asset' class='va-mob-sortrow' ") . button(_("Quantity"), " type='button' th='sorted' c='Qty' class='va-mob-sortrow' "), '', " class='sheet-body va-mob-sortsheet-body' ")
                ...
                .input('hidden', 'ip', $IdParent, "s='d'")
                 .div(
                     div(
                        div($tr, '', "id='SimWalletTable' class='va-mob-list'")
                        .div("<table class='va-dt-table'>".$trHead."<tbody>".$trDt."</tbody></table>", '', " class='dt-card va-dt-only' ")
                     ,'',' class="content" ')
                ,'listForm',' class="ac-list" ')
                .$this->hookListBottom
                .$bottomRow
            , 'SimWalletListForm', " class='va-mob proto-app' data-model='SimWallet' data-table='SimWallet' data-gc-db='sim_wallet' data-ui='".$this->uiTabsId."' ");
        ...
        var __ab=document.getElementById('addSimWalletAutoc');
        if(__ab){
    ...
    */
    public function setCreateDefaultsSimWallet(array $data): SimWallet
    {

        unset($data['IdSimWallet']);
        $e = new SimWallet();
    ...
    *	Make sure default value are set before save
    */
    public function setUpdateDefaultsSimWallet(array $data): ?SimWallet
    {


        $e = $_SESSION[_AUTH_VAR]->loadPkScoped(SimWalletQuery::class, json_decode($data['i']), 'SimWallet', 'w');
    ...
    /**
     * Produce a formated form of SimWallet
    ...
    */
    ...
    public function getEditForm($id, $uiTabsId = 'tabsContain', $data=[], $error=[], $jsElement='', $jsElementType=''): array
    {
        ...
        if($_SESSION[_AUTH_VAR]->hasRights('SimWallet', 'a') && !$this->setReadOnly) {
            $this->formAddButton = htmlLink(_("Add new"), 'Javascript:;' , "id='addSimWalletForm' title='"._('Add')."' class='button-link-blue add-button'");
            $this->bindEditJs = "";
        ...
        if($id && empty($data['reload'])) {
            ...
            if (!$_SESSION[_AUTH_VAR]->isRoot()) {
                ...
                $_SESSION[_AUTH_VAR]->applyOwnerGroupScope($q, $_SESSION[_AUTH_VAR]->hasRights('SimWallet', 'r'));
            }
        ...
        if($dataObj == null){
            $this->SimWallet['isNew'] = 'yes';
            $dataObj = new SimWallet();
        }
...
$this->fields['SimWallet']['Asset']['html'] = stdFieldRow(_("Asset"), input('text', 'Asset', htmlentities((string)($dataObj->getAsset() ?? '')), "   placeholder='".str_replace("'","&#39;",_('Asset'))."' size='15'  v='ASSET' s='d' class='req'  ")."", 'Asset', "", $this->commentsAsset, $this->commentsAsset_css, ' half', ' ', 'no', 'v2');
$this->fields['SimWallet']['Qty']['html'] = stdFieldRow(_("Quantity"), input('number', 'Qty', $dataObj->getQty(), "  placeholder='".str_replace("'","&#39;",_('Quantity'))."'  v='QTY' size='10' s='d' class='req'"), 'Qty', "", $this->commentsQty, $this->commentsQty_css, ' half', ' ', 'no', 'v2');


        $this->lockFormField(array(0=>'Asset',1=>'Qty',2=>'IdCreation',3=>'IdModification',4=>'IdGroupCreation',), $dataObj);
        ...
        if(!$this->setReadOnly){
            ...
            if( ($_SESSION[_AUTH_VAR]->hasRights('SimWallet','a') && !$id) || ($_SESSION[_AUTH_VAR]->hasRights('SimWallet','w') && $id) ){
                $this->formSaveBtn = input('button', 'saveSimWallet', _('Save'),' class="nav-save can-save"');
            }
            $this->formSaveBar = div( input('hidden', 'formChangedSimWallet','', 'j="formChanged"')
                            .input('hidden', 'idPk', urlencode($id), "s='d'")
                        .input('hidden', 'IdSimWallet', $dataObj->getIdSimWallet(), " s='d' pk").input('hidden', 'IdGroupCreation', $dataObj->getIdGroupCreation(), " s='d' nodesc").input('hidden', 'IdCreation', $dataObj->getIdCreation(), " s='d' nodesc").input('hidden', 'IdModification', $dataObj->getIdModification(), " s='d' nodesc")
        ...
        if (!empty($this->formCustomTabs)) {
            throw new \LogicException('SimWallet: addFormTab() needs add_tab_columns (without add_field_groups) on the table — there is no tab strip to put the tab in');
        }
        ...
        .form(
            div(
                div(
                    div(
                        href('<i class="ri-arrow-left-s-line"></i>'._('Paper Wallet'), _SITE_URL.'SimWallet', "class='nav-btn'")
                        .div(
                            span(_('Paper Wallet'), "class='nav-title-type'")
                        , '', "class='nav-title'")
                        .$this->formSaveBtn
                        .href('<i class="ri-close-line"></i>', _SITE_URL.'SimWallet', "class='nav-btn nav-close' title='"._('Close')."' aria-label='"._('Close')."'")
                    ,'',"class='form-nav'")
                    .$childTabsHtml
                ,'',"class='sw-header'")
                .$identityHtml
...
$this->fields['SimWallet']['Asset']['html']
.$this->fields['SimWallet']['Qty']['html'] ."</div>",'',"class='form-card'")

                    .$this->formSaveBar
                    .$this->hookFormInnerBottom
                    .$childPannelHtml
                ,"divCntSimWallet", "class='sw-body form-scroll divStdform' CntTabs=1 ".$this->ccStdFormOptions)
                .$_gcFooterHtml
            ,'',"class='sw-drawer is-open proto-form'")
        , "id='formSimWallet' class='mainForm formContent' ")
        .$this->hookFormBottom;
    ...
    {
        if($fields === 'all') {
            $fields = array_keys($this->fields['SimWallet']);
        ...
        foreach($fields as $field) {
            ...
            $this->fields['SimWallet'][$field]['html'] = $this->fieldsRo['SimWallet'][$field]['html'] ?? '';
        }
    }
    ...
    private function gcBuildFieldRo($field, $dataObj)
    {
        switch($field) {
            case 'Asset':
        $this->fieldsRo['SimWallet']['Asset']['html'] = stdFieldRow(_("Asset"), div( htmlspecialchars((string)($dataObj->getAsset()), ENT_QUOTES), 'Asset_label' , "class='readonly ro-value' s='d'")
                .input('hidden', 'Asset', $dataObj->getAsset(), "s='d'"), 'Asset', "", $this->commentsAsset, $this->commentsAsset_css, 'readonly half', ' ', 'no', 'v2');

            break;
            case 'Qty':
        $this->fieldsRo['SimWallet']['Qty']['html'] = stdFieldRow(_("Quantity"), div( htmlspecialchars((string)($dataObj->getQty()), ENT_QUOTES), 'Qty_label' , "class='readonly ro-value' s='d'")
